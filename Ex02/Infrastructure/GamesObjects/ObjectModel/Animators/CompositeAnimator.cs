...
namespace Infrastructure.GamesObjects.ObjectModel.Animators
{
    public class CompositeAnimator : SpriteAnimator
    {
        ...
        public CompositeAnimator(Sprite i_BoundSprite)
            : this("AnimatorManager", TimeSpan.Zero, i_BoundSprite, new SpriteAnimator[] { })
        {
            this.Enabled = false;
        }
        ...
        public void Add(SpriteAnimator i_Animation)
        {
            ...
            if (!r_AnimationsDictionary.ContainsKey(i_Animation.Name))
            {
        ...
        {
            for (int i = 0; i < r_AnimationsList.Count; i++)
            {
                r_AnimationsList[i].Update(i_GameTime);
            }
        }
    }
}
